...
- Свежий локальный gate после R6 first worker-pool slice и review repairs: **88/88 OK**, `compileall` и `git diff --check` прошли. (Системный `py` launcher сломан; использован bundled Codex Python.)
...
- **R6 first slice**: добавлен `BoundedWorkerPool` с bounded worker/queue capacity, caller-scoped idempotency, isolated concurrent job state и queued/running cooperative cancellation; cancelled jobs удерживают physical model slot до завершения executor. Durable store, MCP Tasks и model-specific scheduling ещё не заявлены.
...
- **MCP/R6 next**: transport-neutral core seam, первый process-bound stdio slice и bounded in-memory worker pool готовы; дальше нужны официальный MCP SDK/conformance, durable Tasks lifecycle и model-specific queue scheduling. Apply и remote HTTP не входят в эти slices.
...
- `local_coding_agent/worker_pool.py` — bounded in-memory queue, job state, idempotency и cooperative cancellation.
